...
### 3.0 Gates
Every phase boundary is a hard gate: Phase N+1 cannot begin until Phase N produced its artifact AND passed its review (§6). Protocol: produce artifact → update experiment log → review → resolve Category A → advance. Never skip an artifact, even under context pressure — write it and stop cleanly. Phase CLAUDE.md templates (from `templates/`) are the runtime entry points.
...
- **The "nodding physicist" test.** At every phase (incl. 4a), would a physicist find every step well-motivated, every number supported, every limitation honestly assessed with evidence that improvement was attempted? A 4a AN should read like a draft paper.
- **Solve problems, don't accept limitations.** Poor tagger purity → alternative variables / contamination correction; uncalibrated variable → data-driven calibration before a flat systematic; missing published data → digitize/HEPData/RAG; dominant systematic → better evaluation method / decompose norm vs shape (a 10×-too-large systematic is usually an evaluation problem). Accept a limitation only when (a) ≥1 concrete approach was tried, (b) it failed for a specific documented reason, (c) solving needs multi-day compute or unavailable resources. A limitation without evidence of attempted improvement is Category B.
- **Every measurement needs context** (compare to reference, state consistency + why, what question it answers, what it can resolve) and **cross-checks are the physics content** (alternative method/selection, subperiod stability, published overlay with chi2). **Published-overlay is mandatory at 4a** (deferring to Phase 5 is Category B). **Implement feasible improvements now**, don't defer to Future Directions (§12).

---

### Phase 1: Strategy
Written strategy a collaboration reviewer could approve. The agent must:
- Query the corpus for detector capabilities, prior work, datasets; identify signal, backgrounds (irreducible/reducible/instrumental), discriminating variables.
- **Technique justification** against alternatives; **selection-approach exploration plan** — identify ≥2 candidate approaches and commit to trying both in Phase 3 with a comparison metric (a single approach is acceptable only if alternatives are documented infeasible AND the review validates it).
...
- Identify 2–3 reference analyses, tabulate their systematic programs, and **extract their published numerical results** into the artifact (binding §6.8 comparison targets at 4c — do not defer to Phase 5).
...
- **Variation sizing:** every systematic motivated by a measurement/calibration/published uncertainty, not a round number ("±50% on background" is Category A unless 50% IS the measured uncertainty). **No borrowed flat systematics** — vary the source and re-run the chain; flat is acceptable only when confirmed subdominant AND propagation infeasible AND the value is cited (all three documented). A perfectly flat relative shift across all bins is physically suspect.
- **No inflated uncertainties** (as harmful as underestimation): re-evaluate don't transfer (an MC value > 2× the data value is inflation unless justified); no "conservative" rounding up; state resolving power; cross-check the total vs naive √N scaling from a published result; pull-distribution sanity across ≥5 quantities (~32% should exceed 1σ — all pulls < 0.5σ with large uncertainties signals inflation). When pull RMS < 0.7, investigate overcoverage and state the dominant source (Category A if not discussed).
...
- Construct the binned likelihood (Asimov data + systematic NPs); validate NP pulls small, fit converges, results sensible. Signal injection (searches) or closure (measurements). **GoF: chi²/ndf AND a toy-based saturated-model p-value.** **Fit boundary check:** no fitted parameter within 1% of a boundary (else the uncertainty is a lower bound — widen/refit or document saturation).
- **Per-systematic documentation** in running prose (origin, evaluation method, numerical impact, interpretation — not bold-label form headings). **Phase-1 traceability:** re-read STRATEGY.md, every committed source implemented or formally downscoped [D] (silent absence is Category A). **Zero-impact sanity:** any exactly-zero variation → verify the varied input is non-trivial. **Systematic self-check** per variation: varied quantity actually changes, impact non-zero, expected sign, consistent eval level (gen/reco), propagated not borrowed.
...
**Artifact:** `ANALYSIS_NOTE_5_v1.md` + compiled PDF + `results/`. **Review:** 1 reviewer. See `04-output.md` for the full AN spec.
...
**Artifacts are the only handoff** — no conversation history, no shared variables; each session starts from artifacts + instructions. Three layers per agent: (1) bird's-eye framing (~1 page: physics prompt, analysis type, current phase, conventions, end goal); (2) relevant methodology sections (~2–5 pages, per role — e.g. Phase-4 executor: §3 Phase 4, §4, §5, §7, §11, plotting; reviewer: §6 + the phase + conventions + checklist); (3) upstream artifacts (~2–10 pages). Budget ~20–30 pages; summarize artifacts over ~5 pages; consult the experiment log on demand. When context pressure mounts, write the artifact and stop cleanly — never skip an artifact to save context.
