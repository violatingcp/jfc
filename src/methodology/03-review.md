...
- **Ph4a (lean scope — light check only):** confirm the fit of the final signal-extraction distribution converges on the correct Asimov and recovers the injected inputs (μ, m_H); circular/tautological fitted parameters; §6.8. No exhaustive checklist at 4a; the number-consistency gate does not run here.
- **Ph4b:** number-consistency gate (first applied) — the final signal-extraction distribution's bin contents/yields and the quoted μ/m_H match the latest machine outputs (stale = correctness error); compatible with 4a; §6.8.
